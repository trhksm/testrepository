...
void init() {
    ...
    glEnable(GL_COLOR_MATERIAL);  // 色を有効にする
    glColorMaterial(GL_FRONT, GL_AMBIENT_AND_DIFFUSE); // 色反映方法
}
...
void display() {
    ...
    gluLookAt(eye[0], eye[1], eye[2],
    	      pov[0], pov[1], pov[2],
              up[0], up[1], up[2]);
    for(int i = 0; i < 3; i++){light[i] = eye[i] + offset_up * up[i];}
